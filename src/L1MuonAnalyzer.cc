...
{
  hMuPerBx_ = fs->make<TH1I>(("hMuPerBx"+label).c_str(), (titlePrefix+" muons per BX").c_str(), 15, -7, 8);
}
...
TfHistos::FillMuon(const l1t::RegionalMuonCand& muon)
{
  ...
  hHwGlobalPhi_->Fill(l1t::MicroGMTConfiguration::calcGlobalPhi(muon.hwPhi(), muon.trackFinderType(), muon.processor()));
...
L1MuonAnalyzer::analyzeRegionalMuonCand(const l1t::RegionalMuonCandBxCollection& muons, TfHistos& histos, const std::string& label)
{
  ...
  for (int bx = muons.getFirstBX(); bx <= muons.getLastBX(); ++bx) {
    if (debug_) std::cout << "BX " << bx << ": " << muons.size(bx) << " muons" << std::endl;
    unsigned int ctr = 0;
